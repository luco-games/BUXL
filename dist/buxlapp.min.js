...
BuxlViewPrototype.prototype.registerEvents = function registerEvents() {
    for (var i = 0; i < this.events.length; i++) {
        ...
        if (this.events[i].target === "window") {
            ...
            for (var j = 0; j < elements.length; j++) this.registerEventsPerTrigger(elements[j], this.events[i].f, triggers);
        }
    }
};
...
BuxlGameModel.prototype.setCurrentBuxl = function setCurrentBuxl(gameHash) {
    ...
    if (gameHash && !this.buxls[gameHash]) return false;
...
BuxlNavigationController.prototype.toggleFavorite = function toggleFavorite(gameHash) {
    ...
    if (this.currentRoute === "buxl") render = this.view.render.bind(this.view, dataModel, true);else if (this.currentRoute === "favorites") render = this.view.renderOnList.bind(this.view, dataModel, true);
...
BuxlRoutingController.prototype.doRouting = function doRouting(e) {
    ...
    if (!route) var route = window.location.hash;

    route = route.substring(2).split('/');

    if (route.length < 2) this.routeDefault();else this.route(route[0], route[1]);
};

BuxlRoutingController.prototype.route = function route(route, gameHash) {
    var currentRoute = this.routes[route];

    if (typeof currentRoute === 'undefined') this.routeDefault();

    for (var i = 0; i < currentRoute.length; i++) if (currentRoute[i]) currentRoute[i].route(route, gameHash);
};
...
BuxlRoutingController.prototype.init = function init() {
    for (var controller in this.controllers) this.controllers[controller].init(this.doRouting.bind(this));
